...
import com.thizthizzydizzy.treefeller.menu.modify.*;
...
public abstract class Option<E> {
    ...
    public static OptionBoolean DEFAULT_ENABLED = new OptionBoolean("Default Enabled", true, false, false, true, true) {
        @Override
        public String getDesc(boolean ingame) {
            return "If set to false, the tree feller will be toggled off for each player by default (as with " +
             "/treefeller toggle)" + (ingame ? "\nOnly takes effect on reload/restart" : "");
        }
    ...
    public static Option<Integer> SCAN_DISTANCE = new Option<Integer>("Scan Distance", true, true, false, 256) {
        ...
        @Override
        public String getDesc(boolean ingame) {
            return "How far should the plugin scan for logs? (If a tree is larger, only the part within this distance" +
 " will be felled)";
        }
        ...
        public void openGlobalModifyMenu(MenuGlobalConfiguration parent) {
            parent.open(new MenuModifyInteger(parent, parent.plugin, parent.player, name, 0, Integer.MAX_VALUE, false
        , globalValue, (value) -> {
                globalValue = value;
            }));
        }
        ...
        public void openToolModifyMenu(MenuToolConfiguration parent, Tool tool) {
            parent.open(new MenuModifyInteger(parent, parent.plugin, parent.player, name, 0, Integer.MAX_VALUE, true,
             toolValues.get(tool), (value) -> {
 ...
 toolValues.get(tool), (value) -> {
    ...
    public static Option<Integer> LEAF_BREAK_RANGE = new Option<Integer>("Leaf Break Range", true, true, true, 6) {
        ...
        @Override
        public String getDesc(boolean ingame) {
            return "How far away from logs should leaf blocks be destroyed? (set to 0 to prevent leaves from being " +
             "destroyed) (Values over 6 are useless for vanilla trees, as these leaves would naturally decay anyway)";
        }
    ...
    public static Option<Integer> REQUIRED_LOGS = new Option<Integer>("Required Logs", true, true, true, 4) {
        ...
        @Override
        public DebugResult doCheckTrunk(TreeFeller plugin, Tool tool, Tree tree,
HashMap<Integer, ArrayList<Block>> blocks, Block block) {
 ...
 treeValues.get(tree), (value) -> {
    ...
    public static Option<Integer> REQUIRED_LEAVES = new Option<Integer>("Required Leaves", true, true, true, 10) {
        ...
        @Override
        public DebugResult doCheckTree(TreeFeller plugin, Tool tool, Tree tree,
            HashMap<Integer, ArrayList<Block>> blocks, int leaves) {
 ...
 , globalValue, (value) -> {
        ...
        public Integer get(Tool tool, Tree tree) {
            ...
            if (getValue(tool) == null && getValue(tree) == null && getValue() != null)
                val = Math.min(val == null ? Integer.MAX_VALUE : val, getValue());
    ...
    public static Option<Integer> MAX_HEIGHT = new Option<Integer>("Max Height", true, true, true, 5) {
        ...
        @Override
        public String getDesc(boolean ingame) {
            return "How far from the bottom can you cut down a tree? (Prevents you from cutting it down from the top)" +
             " 1 = bottom block";
        }
        ...
        @Override
        public void openGlobalModifyMenu(MenuGlobalConfiguration parent) {
            parent.open(new MenuModifyInteger(parent, parent.plugin, parent.player, name, 1, Integer.MAX_VALUE, false
            , globalValue, (value) -> {
                globalValue = value;
            }));
        }
        ...
        @Override
        public void openToolModifyMenu(MenuToolConfiguration parent, Tool tool) {
            parent.open(new MenuModifyInteger(parent, parent.plugin, parent.player, name, 1, Integer.MAX_VALUE, true,
             toolValues.get(tool), (value) -> {
                if (value == null) toolValues.remove(tool);
                else toolValues.put(tool, value);
            }));
 ...
 treeValues.get(tree), (value) -> {
    ...
    public static OptionBoolean ALLOW_PARTIAL = new OptionBoolean("Allow Partial", true, true, true, false) {
        @Override
        public String getDesc(boolean ingame) {
            return "Should trees be able to be partially cut down if the tool has insufficient durability? It cannot " +
             "be guaranteed what part of the tree will be cut down!";
        }
    ...
    public static OptionBoolean IGNORE_LEAF_DATA = new OptionBoolean("Ignore Leaf Data", true, false, true, false) {
        @Override
        public String getDesc(boolean ingame) {
            return "If set to true, leaves' blockdata will be ignored. This should only be set if custom trees' " +
             "leaves are not being destroyed when they should.";
        }
    ...
    public static OptionBoolean REQUIRE_CROSS_SECTION = new OptionBoolean("Require Cross Section", true, true, true,
            false) {
        @Override
        public DebugResult doCheckTrunk(TreeFeller plugin, Tool tool, Tree tree,
         HashMap<Integer, ArrayList<Block>> blocks, Block block) {
                ...
                for (int x = -1; x <= 1; x++) {
                    for (int z = -1; z <= 1; z++) {
                        ...
                        if (tree.trunk.contains(block.getRelative(x, 0, z).getType())) {
                            if (toolValues.get(tool) == null && treeValues.get(tree) == null && Objects.equals(globalValue, true))
                                return new DebugResult(this, GLOBAL, block.getX() + x,
                                 block.getY() + " " + block.getZ() + z);
                            if (Objects.equals(toolValues.get(tool), true))
                                return new DebugResult(this, TOOL, block.getX() + x,
 block.getY() + " " + block.getZ() + z);
                            if (Objects.equals(treeValues.get(tree), true))
                                return new DebugResult(this, TREE, block.getX() + x,
                                 block.getY() + " " + block.getZ() + z);
                        }
                    }
                }
        ...
        @Override
        public String getDesc(boolean ingame) {
            return "Should trees larger than 1x1 require an entire horizontal cross-section to be mined before the " +
             "tree fells? (Works for up to 2x2 trees)";
        }

        @Override
        public String[] getDebugText() {
            return generateDebugText("A full cross-section has not been cut$ at ({0}, {1}, {2})", "A full " +
                    "cross-section has been cut");
        }
    ...
    public static OptionBoolean FORCE_DISTANCE_CHECK = new OptionBoolean("Force Distance Check", true, false, true,
     false) {
        @Override
        public String getDesc(boolean ingame) {
            return "If set to true, all non-leaf-block leaves will be distance-checked to make sure they belong to " +
                    "the tree being felled (ex. mushrooms or nether 'tree' leaves)\n"
    ...
    public static Option<HashSet<Material>> BANNED_LOGS = new Option<HashSet<Material>>("Banned Logs", true, true,
     true, new HashSet<>()) {
        @Override
        public String getDesc(boolean ingame) {
            return "Which trunk blocks should prevent a tree from being felled? (Intended to help prevent player " +
            "structures from being cut down)\n(For this to work, these must also be included in the tree trunk " +
              "materials)\nEx. planks/glass";
        }
        ...
        @Override
        public void openGlobalModifyMenu(MenuGlobalConfiguration parent) {
            parent.open(new MenuModifyMaterialSet(parent, parent.plugin, parent.player, name, true, "block",
             globalValue, (material) -> {
        ...
        @Override
        protected DebugResult doCheckTrunk(TreeFeller plugin, Tool tool, Tree tree, HashMap<Integer,
         ArrayList<Block>> blocks, Block block) {
    ...
    public static Option<HashSet<Material>> BANNED_LEAVES = new Option<HashSet<Material>>("Banned Leaves", true, true
 , true, new HashSet<>()) {
        @Override
        public String getDesc(boolean ingame) {
            return "Which blocks should prevent a tree from being felled? (Intended to help prevent player structures" +
" from being cut down)\n(For this to work, these must also be included in the tree leaf materials)\nEx. " +
        ...
        @Override
        protected DebugResult doCheckTree(TreeFeller plugin, Tool tool, Tree tree,
         HashMap<Integer, ArrayList<Block>> blocks, int leaves) {
            for (ArrayList<Block> blox : blocks.values()) {
                for (Block blok : blox) {
                    Material mat = blok.getType();
    ...
    public static Option<Integer> MAX_HORIZONTAL_TRUNK_PILLAR_LENGTH = new Option<Integer>("Max Horizontal Trunk " +
                        "Pillar Length", true, true, true, 6) {
        @Override
        public String getDesc(boolean ingame) {
            return "What is the maximum number of trunk blocks that may be in a horizontal line?\n(This is to help " +
                    "detect structures with long horizontal pillars of logs)";
        }
        ...
        protected DebugResult doCheckTrunk(TreeFeller plugin, Tool tool, Tree tree, HashMap<Integer,
         ArrayList<Block>> blocks, Block block) {
            ...
            while (!xAxis.isEmpty()) {
                ...
                xAxis.removeAll(line);//don't need to recheck the same line many times. Maybe this will actually make
                    // it faster even with all the nonsense above
            }
            ...
            while (!zAxis.isEmpty()) {
                ...
                actual = Math.max(actual, line.size());
                zAxis.removeAll(line);//don't need to recheck the same line many times. Maybe this will actually make
        // it faster even with all the nonsense above
            }
            if (toolValues.get(tool) == null && treeValues.get(tree) == null && globalValue != null && actual > globalValue)
                return new DebugResult(this, GLOBAL, actual, globalValue);
            if (treeValues.containsKey(tree) && actual > treeValues.get(tree))
                return new DebugResult(this, TREE, actual, treeValues.get(tree));
            if (toolValues.containsKey(tool) && actual > toolValues.get(tool))
                return new DebugResult(this, TOOL, actual, toolValues.get(tool));
            return new DebugResult(this, SUCCESS);
        }

        @Override
        public String[] getDebugText() {
            return generateDebugText("Found a horizontal trunk pillar that was too long$! {0}>{1}", "No excessive " +
             "horizontal trunk pillars found");
        }
    };
    public static Option<Integer> MAX_TRUNKS = new Option<Integer>("Max Trunks", true, true, true, 1) {
        @Override
        public String getDesc(boolean ingame) {
            return "What is the maximum number of trunks a tree may have?\nNote that the trunks are counted at the " +
             "level at which the tree is cut; not at the base of the tree\nSimilarly to leave-stump, this may include" +
 " low-hanging leaves";
        }
        ...
        protected DebugResult doCheckTrunk(TreeFeller plugin, Tool tool, Tree tree, HashMap<Integer,
            ...
            if (toolValues.get(tool) == null && treeValues.get(tree) == null && globalValue != null && value > globalValue)
                return new DebugResult(this, GLOBAL, value, globalValue);
            if (treeValues.containsKey(tree) && value > treeValues.get(tree))
                return new DebugResult(this, TREE, value, treeValues.get(tree));
            if (toolValues.containsKey(tool) && value > toolValues.get(tool))
                return new DebugResult(this, TOOL, value, toolValues.get(tool));
            return new DebugResult(this, SUCCESS);
        }
        ...
        private ArrayList<Block> getTrunkBit(ArrayList<Block> theWholeTrunkSlice, Block startingBlock,
         boolean diagonal) {
            //layer zero
    ...
    public static OptionBoolean USE_INVENTORY_SAPLINGS = new OptionBoolean("Use Inventory Saplings", true, true, true
        , false) {
        @Override
        public String getDesc(boolean ingame) {
    ...
    public static Option<Integer> SPAWN_SAPLINGS = new Option<Integer>("Spawn Saplings", true, true, true, 0) {
        ...
        @Override
        public void openGlobalModifyMenu(MenuGlobalConfiguration parent) {
            parent.open(new MenuModifySpawnSaplings(parent, parent.plugin, parent.player, false, globalValue,
(value) -> {
                globalValue = value;
            }));
        }
        ...
        @Override
        public void openToolModifyMenu(MenuToolConfiguration parent, Tool tool) {
            parent.open(new MenuModifySpawnSaplings(parent, parent.plugin, parent.player, true, toolValues.get(tool),
        (value) -> {
                if (value == null) toolValues.remove(tool);
                else toolValues.put(tool, value);
            }));
        ...
        @Override
        public void openTreeModifyMenu(MenuTreeConfiguration parent, Tree tree) {
            parent.open(new MenuModifySpawnSaplings(parent, parent.plugin, parent.player, true, treeValues.get(tree),
            (value) -> {
                if (value == null) treeValues.remove(tree);
                else treeValues.put(tree, value);
            }));
    ...
    public static Option<HashSet<Material>> SAPLING = new Option<HashSet<Material>>("Sapling", false, false, true,
        null) {
        @Override
        public HashSet<Material> load(Object o) {
            return loadMaterialSet(o);
        ...
        @Override
        public void openGlobalModifyMenu(MenuGlobalConfiguration parent) {
            parent.open(new MenuModifyMaterialSet(parent, parent.plugin, parent.player, name, false, "block",
globalValue, (material) -> {
                return material.isBlock();
            }, (value) -> {
                globalValue = value;
    ...
    public static Option<Integer> SAPLING_TIMEOUT = new Option<Integer>("Sapling Timeout", true, true, true, 50) {
        ...
        @Override
        public String getDesc(boolean ingame) {
            return "The amount of delay a sapling may take to respawn (if spawn saplings is set to 1, saplings will " +
                "be spawned only after this amount of time, but will still be immediately planted upon dropping from " +
              "leaves)";
        }
 ...
 treeValues.get(tree), (value) -> {
    ...
    public static Option<HashSet<Material>> GRASS = new Option<HashSet<Material>>("Grass", true, false, true,
                defaultGrasses) {
        @Override
        public HashSet<Material> load(Object o) {
            return loadMaterialSet(o);
    ...
    public static Option<HashSet<Material>> ROOTS = new Option<HashSet<Material>>("Roots", false, false, true, null) {
        ...
        @Override
        public String getDesc(boolean ingame) {
            return "Roots can be used to cut down a tree when you can't reach the trunk. (The nearest tree will be " +
             "attempted to be cut down)";
        }
...
toolValues.get(tool), (material) -> {
    ...
    public static Option<FellBehavior> LOG_BEHAVIOR = new Option<FellBehavior>("Log Behavior", true, true, true,
     FellBehavior.BREAK) {
        @Override
        ...
        @Override
        public void openGlobalModifyMenu(MenuGlobalConfiguration parent) {
            parent.open(new MenuModifyFellBehavior(parent, parent.plugin, parent.player, name, false, globalValue,
             FellBehavior.values(), (value) -> {
                globalValue = value;
            }));
        }
        ...
        @Override
        public void openToolModifyMenu(MenuToolConfiguration parent, Tool tool) {
            parent.open(new MenuModifyFellBehavior(parent, parent.plugin, parent.player, name, true,
             toolValues.get(tool), FellBehavior.values(), (value) -> {
                if (value == null) toolValues.remove(tool);
                else toolValues.put(tool, value);
            }));
        }
        ...
        public void openTreeModifyMenu(MenuTreeConfiguration parent, Tree tree) {
            parent.open(new MenuModifyFellBehavior(parent, parent.plugin, parent.player, name, true,
             treeValues.get(tree), FellBehavior.values(), (value) -> {
                if (value == null) treeValues.remove(tree);
                else treeValues.put(tree, value);
            }));
        }
    };
    public static Option<FellBehavior> LEAF_BEHAVIOR = new Option<FellBehavior>("Leaf Behavior", true, true, true,
     FellBehavior.BREAK) {
        @Override
        public FellBehavior load(Object o) {
            if (o instanceof FellBehavior) return (FellBehavior) o;
...
toolValues.get(tool), FellBehavior.values(), (value) -> {
 ...
 treeValues.get(tree), FellBehavior.values(), (value) -> {
    ...
    public static Option<Float> FALL_HURT_AMOUNT = new Option<Float>("Fall Hurt Amount", true, true, true, 2.0f) {
        ...
        @Override
        public String getDesc(boolean ingame) {
            return "How much damage should falling blocks deal per block fallen? (Only applies to FALL_HURT fell " +
             "behaviors)";
        }
        ...
        @Override
        public void openGlobalModifyMenu(MenuGlobalConfiguration parent) {
            parent.open(new MenuModifyFloat(parent, parent.plugin, parent.player, name, 0f, Integer.MAX_VALUE, 1,
 ...
 true, toolValues.get(tool), (value) -> {
    ...
    public static Option<Integer> FALL_HURT_MAX = new Option<Integer>("Fall Hurt Max", true, true, true, 40) {
        ...
        @Override
        public String getDesc(boolean ingame) {
            return "What is the maximum amount of damage a falling block may deal upon landing? (Only applies to " +
             "FALL_HURT fell behaviors)";
        }
...
, globalValue, (value) -> {
    ...
    public static Option<DirectionalFallBehavior> DIRECTIONAL_FALL_BEHAVIOR = new Option<DirectionalFallBehavior>(
            "Directional Fall Behavior", true, true, true, DirectionalFallBehavior.RANDOM) {
        @Override
        ...
        @Override
        public void openGlobalModifyMenu(MenuGlobalConfiguration parent) {
            parent.open(new MenuModifyDirectionalFallBehavior(parent, parent.plugin, parent.player, name, false,
             globalValue, DirectionalFallBehavior.values(), (value) -> {
                globalValue = value;
            }));
        }
        ...
        @Override
        public void openToolModifyMenu(MenuToolConfiguration parent, Tool tool) {
            parent.open(new MenuModifyDirectionalFallBehavior(parent, parent.plugin, parent.player, name, true,
             toolValues.get(tool), DirectionalFallBehavior.values(), (value) -> {
                if (value == null) toolValues.remove(tool);
                else toolValues.put(tool, value);
            }));
        }
        ...
        public void openTreeModifyMenu(MenuTreeConfiguration parent, Tree tree) {
            parent.open(new MenuModifyDirectionalFallBehavior(parent, parent.plugin, parent.player, name, true,
             treeValues.get(tree), DirectionalFallBehavior.values(), (value) -> {
                if (value == null) treeValues.remove(tree);
                else treeValues.put(tree, value);
            }));
        }
    };
    public static Option<HashSet<Material>> OVERRIDABLES = new Option<HashSet<Material>>("Overridables", true, true,
     true, defaultOverridables) {
        @Override
        public HashSet<Material> load(Object o) {
            return loadMaterialSet(o);
        }
        ...
        @Override
        public String getDesc(boolean ingame) {
            return "This is the list of blocks that may be overridden when a tree falls onto them (e.g air, grass, " +
 "water)\n" +
...
toolValues.get(tool), (material) -> {
    ...
    public static Option<Double> DIRECTIONAL_FALL_VELOCITY = new Option<Double>("Directional Fall Velocity", true,
        true, true, .35d) {
        @Override
        ...
        @Override
        public void openGlobalModifyMenu(MenuGlobalConfiguration parent) {
            parent.open(new MenuModifyDouble(parent, parent.plugin, parent.player, name, Integer.MIN_VALUE,
             Integer.MAX_VALUE, false, globalValue, (value) -> {
                globalValue = value;
            }));
        }
        ...
        public void openToolModifyMenu(MenuToolConfiguration parent, Tool tool) {
            parent.open(new MenuModifyDouble(parent, parent.plugin, parent.player, name, Integer.MIN_VALUE,
    Integer.MAX_VALUE, true, toolValues.get(tool), (value) -> {
                if (value == null) toolValues.remove(tool);
                else toolValues.put(tool, value);
            }));
        }
        ...
        public void openTreeModifyMenu(MenuTreeConfiguration parent, Tree tree) {
            parent.open(new MenuModifyDouble(parent, parent.plugin, parent.player, name, Integer.MIN_VALUE,
             Integer.MAX_VALUE, true, treeValues.get(tree), (value) -> {
                if (value == null) treeValues.remove(tree);
                else treeValues.put(tree, value);
            }));
        }
    };
    public static Option<Double> VERTICAL_FALL_VELOCITY = new Option<Double>("Vertical Fall Velocity", true, true,
     true, .05d) {
        @Override
        public Double load(Object o) {
            return loadDouble(o);
...
Integer.MAX_VALUE, true, toolValues.get(tool), (value) -> {
    ...
    public static Option<Double> EXPLOSIVE_FALL_VELOCITY = new Option<Double>("Explosive Fall Velocity", true, true,
     true, 0d) {
        @Override
        public Double load(Object o) {
            return loadDouble(o);
        ...
        @Override
        public String getDesc(boolean ingame) {
            return "How much explosive sideways velocity should falling blocks get? Velocity is applied away from the" +
             " block that was used to cut down the tree (no velocity will be applied to blocks in the exact center)\n" +
                    "(Only used when log or leaf behavior is set to FALL or similar)";
        }
    ...
    public static Option<Double> RANDOM_FALL_VELOCITY = new Option<Double>("Random Fall Velocity", true, true, true,
     0d) {
        @Override
        public Double load(Object o) {
            return loadDouble(o);
    ...
    public static Option<Integer> FALL_DELAY = new Option<Integer>("Fall Delay", true, true, true, 0) {
        ...
        @Override
        public String getDesc(boolean ingame) {
            return "An extra delay between breaking a block and spawning a falling block. (Only affects FALL " +
             "behaviors)\nThis is intended as a workaround for collision issues between fall behaviors and the " +
        "cutting animation";
        }
 ...
 , globalValue, (value) -> {
    ...
    public static Option<Double> DAMAGE_MULT = new Option<Double>("Damage Mult", true, true, true, 1d) {
        ...
        @Override
        public void openGlobalModifyMenu(MenuGlobalConfiguration parent) {
            parent.open(new MenuModifyDouble(parent, parent.plugin, parent.player, name, 0, Integer.MAX_VALUE, 1,
             false, globalValue, (value) -> {
                globalValue = value;
            }));
        }
    ...
    public static OptionBoolean STACKED_TOOLS = new OptionBoolean("Stacked Tools", true, true, false, false) {
        @Override
        public String getDesc(boolean ingame) {
            return "If set to true, stacked tools will be consumed one at a time.\nThis will treat the entire stack " +
             "as one tool, so prevent-breakage will not keep individual tools from breaking, only the whole stack" +
    ...
    public static OptionBoolean LEAVE_STUMP = new OptionBoolean("Leave Stump", true, true, true, false) {
        @Override
        public String getDesc(boolean ingame) {
            return "When a tree is felled, should a stump be left? (The stump consists of any log blocks below the " +
             "point at which the tree was felled)\n" +
                    "This may cause issues with custom trees that have multiple trunks or branches that extend very " +
            "low";
        }
    ...
    public static OptionBoolean ROTATE_LOGS = new OptionBoolean("Rotate Logs", true, true, true, true) {
        @Override
        public String getDesc(boolean ingame) {
            return "When trees fall, should the logs rotate as they fall? (This makes it look more realistic, with " +
                    "logs landing horizontally)\n" +
                    "(Only used when log or leaf behavior is set to FALL or similar)";
        }
    ...
    public static Option<HashMap<Material, Material>> DROP_CONVERSIONS = new Option<HashMap<Material, Material>>(
            "Drop Conversions", true, true, true, defaultDropConversions) {
        @Override
        ...
        @Override
        public String getDesc(boolean ingame) {
            return "A list of drops to convert into other drops when felling." + (ingame ? "" : " add entries like " +
 "this:\n"
                    + "    oak_wood: oak_log\n"
                    + "    oak_fence: stick");
        }
        ...
        public HashMap<Material, Material> get(Tool tool, Tree tree) {
            ...
            if (toolValues.get(tool) == null && treeValues.get(tree) == null && globalValue != null)
                conversions.putAll(globalValue);
    ...
    public static Option<HashMap<Material, Material>> BLOCK_CONVERSIONS = new Option<HashMap<Material, Material>>(
            "Block Conversions", true, true, true, defaultBlockConversions) {
        @Override
        public String writeToConfig(HashMap<Material, Material> value) {
            String s = "";
            if (value == null) return s;
    ...
    public static Option<Double> LEAF_DROP_CHANCE = new Option<Double>("Leaf Drop Chance", true, true, true, 1d) {
        ...
        @Override
        public String getDesc(boolean ingame) {
            return "How often should leaves drop items? Set this to 0.0 to stop leaves from dropping items altogether" +
" (Only works with BREAK behavior)";
        }
    ...
    public static Option<Double> LOG_DROP_CHANCE = new Option<Double>("Log Drop Chance", true, true, true, 1d) {
        ...
        @Override
        public String getDesc(boolean ingame) {
            return "How often should logs drop items? Set this to 0.0 to stop logs from dropping items altogether " +
            "(Only works with BREAK behavior)";
        }
    ...
    , "\n    - ALL") {
        ...
        @Override
        public void openGlobalModifyMenu(MenuGlobalConfiguration parent) {
            parent.open(new MenuModifyEffectList(parent, parent.plugin, parent.player, name, true, globalValue,
             (value) -> {
                globalValue = value;
            }));
        }
        ...
        @Override
        public void openToolModifyMenu(MenuToolConfiguration parent, Tool tool) {
            parent.open(new MenuModifyEffectList(parent, parent.plugin, parent.player, name, true,
             toolValues.get(tool), (value) -> {
                if (value == null) toolValues.remove(tool);
                else toolValues.put(tool, value);
            }));
 ...
 treeValues.get(tree), (value) -> {
    ...
    public static Option<Double> CONSUMED_FOOD_BASE = new Option<Double>("Consumed Food Base", true, true, true, 0d) {
        ...
        @Override
        public String getDesc(boolean ingame) {
            return "How much food should be be consumed upon felling a tree? (This is in addition to per-block " +
             "settings)";
        }
        ...
        public void openGlobalModifyMenu(MenuGlobalConfiguration parent) {
            parent.open(new MenuModifyDouble(parent, parent.plugin, parent.player, name, Integer.MIN_VALUE,
             Integer.MAX_VALUE, 1, false, globalValue, (value) -> {
                globalValue = value;
            }));
        }
        ...
        public void openToolModifyMenu(MenuToolConfiguration parent, Tool tool) {
            parent.open(new MenuModifyDouble(parent, parent.plugin, parent.player, name, Integer.MIN_VALUE,
             Integer.MAX_VALUE, 1, true, toolValues.get(tool), (value) -> {
                if (value == null) toolValues.remove(tool);
                else toolValues.put(tool, value);
            }));
        }
        ...
        public void openTreeModifyMenu(MenuTreeConfiguration parent, Tree tree) {
            parent.open(new MenuModifyDouble(parent, parent.plugin, parent.player, name, Integer.MIN_VALUE,
             Integer.MAX_VALUE, 1, true, treeValues.get(tree), (value) -> {
                if (value == null) treeValues.remove(tree);
                else treeValues.put(tree, value);
            }));
        }
    ...
    public static Option<Double> CONSUMED_FOOD_LOGS = new Option<Double>("Consumed Food Logs", true, true, true, 0d) {
        ...
        @Override
        public String getDesc(boolean ingame) {
            return "How much food should be be consumed per block of log upon felling a tree? (This is in addition to" +
 " the base setting)";
        }
    ...
    public static Option<Double> CONSUMED_FOOD_LEAVES = new Option<Double>("Consumed Food Leaves", true, true, true,
     0d) {
        @Override
        public Double load(Object o) {
            return loadDouble(o);
        ...
        @Override
        public String getDesc(boolean ingame) {
            return "How much food should be be consumed per block of leaves upon felling a tree? (This is in addition" +
                " to the base setting)";
        }
...
Integer.MAX_VALUE, 1, false, globalValue, (value) -> {
    ...
    public static Option<Double> CONSUMED_HEALTH_BASE = new Option<Double>("Consumed Health Base", true, true, true,
            0d) {
        @Override
        public Double load(Object o) {
            return loadDouble(o);
        ...
        @Override
        public String getDesc(boolean ingame) {
            return "How much health should be be consumed upon felling a tree? (This is in addition to per-block " +
             "settings)";
        }
    ...
    public static Option<Double> CONSUMED_HEALTH_LOGS = new Option<Double>("Consumed Health Logs", true, true, true,
     0d) {
        @Override
        public Double load(Object o) {
            return loadDouble(o);
        ...
        @Override
        public String getDesc(boolean ingame) {
            return "How much health should be be consumed per block of log upon felling a tree? (This is in addition " +
                "to the base setting)";
        }
 ...
 Integer.MAX_VALUE, 1, false, globalValue, (value) -> {
    ...
    public static Option<Double> CONSUMED_HEALTH_LEAVES = new Option<Double>("Consumed Health Leaves", true, true,
     true, 0d) {
        @Override
        public Double load(Object o) {
            return loadDouble(o);
        ...
        @Override
        public String getDesc(boolean ingame) {
            return "How much health should be be consumed per block of leaves upon felling a tree? (This is in " +
 ...
 Integer.MAX_VALUE, 1, true, treeValues.get(tree), (value) -> {
    ...
    //requirements
    public static Option<HashMap<Enchantment, Integer>> REQUIRED_ENCHANTMENTS = new Option<HashMap<Enchantment,
     Integer>>("Required Enchantments", true, true, true, null) {
        @Override
            ...
            if (toolValues.get(tool) == null && treeValues.get(tree) == null && globalValue != null) {
                for (Enchantment e : globalValue.keySet()) {
                    if (axe.getEnchantmentLevel(e) < globalValue.get(e)) {
                        return new DebugResult(this, GLOBAL, e.toString(), axe.getEnchantmentLevel(e),
                         globalValue.get(e));
                    }
                }
            }
        ...
        @Override
        public void openGlobalModifyMenu(MenuGlobalConfiguration parent) {
            parent.open(new MenuModifyEnchantmentMap(parent, parent.plugin, parent.player, name, true, globalValue,
                (value) -> {
                globalValue = value;
            }));
        }
        ...
        @Override
        public void openToolModifyMenu(MenuToolConfiguration parent, Tool tool) {
            parent.open(new MenuModifyEnchantmentMap(parent, parent.plugin, parent.player, name, true,
             toolValues.get(tool), (value) -> {
                if (value == null) toolValues.remove(tool);
                else toolValues.put(tool, value);
            }));
 ...
 treeValues.get(tree), (value) -> {
    ...
    public static Option<HashMap<Enchantment, Integer>> BANNED_ENCHANTMENTS = new Option<HashMap<Enchantment,
     Integer>>("Banned Enchantments", true, true, true, null) {
        @Override
        public HashMap<Enchantment, Integer> load(Object o) {
            if (o instanceof MemorySection) {
                HashMap<Enchantment, Integer> enchantments = new HashMap<>();
        ...
        @Override
        public String[] getDebugText() {
            return generateDebugText("Tool contains banned enchantment$: {0} ({1}>{2})", "No banned enchantments " +
             "found");
        }
...
toolValues.get(tool), (value) -> {
 ...
 treeValues.get(tree), (value) -> {
    ...
    public static Option<Short> MIN_DURABILITY = new Option<Short>("Min Durability", true, true, true, null) {
        ...
        public DebugResult doCheck(TreeFeller plugin, Tool tool, Tree tree, Block block, Player player, ItemStack axe) {
            ...
            if (axe.getType().getMaxDurability() > 0) {
                ...
                if (toolValues.containsKey(tool)) {
                    if (durability < toolValues.get(tool))
                        return new DebugResult(this, TOOL, durability, toolValues.get(tool));
                }
                if (treeValues.containsKey(tree)) {
                    if (durability < treeValues.get(tree))
                        return new DebugResult(this, TREE, durability, treeValues.get(tree));
                }
            }
            return new DebugResult(this, SUCCESS);
        }
        ...
        @Override
        public String[] getDebugText() {
            return generateDebugText("Tool durability is less than minimum allowed$: {0}<{1}", "Tool meets minimum " +
        ...
        @Override
        public void openGlobalModifyMenu(MenuGlobalConfiguration parent) {
            parent.open(new MenuModifyShort(parent, parent.plugin, parent.player, name, (short) 0, Short.MAX_VALUE,
             true, globalValue, (value) -> {
                globalValue = value;
            }));
        }
...
true, treeValues.get(tree), (value) -> {
    ...
    public static Option<Short> MAX_DURABILITY = new Option<Short>("Max Durability", true, true, true, null) {
        ...
        @Override
        public String[] getDebugText() {
            return generateDebugText("Tool durability is greater than maximum allowed: {0}>{1}", "Tool meets maximum " +
             "durability requirement");
        }
...
true, treeValues.get(tree), (value) -> {
    ...
    public static Option<Float> MIN_DURABILITY_PERCENT = new Option<Float>("Min Durability Percent", true, true, true
    , null) {
        @Override
        public Float load(Object o) {
            return loadFloat(o);
        ...
        public DebugResult doCheck(TreeFeller plugin, Tool tool, Tree tree, Block block, Player player, ItemStack axe) {
            ...
            if (axe.getType().getMaxDurability() > 0) {
                if (toolValues.get(tool) == null && treeValues.get(tree) == null && globalValue != null) {
                    if (durabilityPercent < globalValue)
                        return new DebugResult(this, GLOBAL, Math.round(durabilityPercent * 1000) / 10f,
                         Math.round(globalValue * 1000) / 10f);
                }
                if (toolValues.containsKey(tool)) {
                    if (durabilityPercent < toolValues.get(tool))
                        return new DebugResult(this, TOOL, Math.round(durabilityPercent * 1000) / 10f,
                         Math.round(toolValues.get(tool) * 1000) / 10f);
                }
                if (treeValues.containsKey(tree)) {
                    if (durabilityPercent < treeValues.get(tree))
                        return new DebugResult(this, TREE, Math.round(durabilityPercent * 1000) / 10f,
                         Math.round(treeValues.get(tree) * 1000) / 10f);
                }
            }
            return new DebugResult(this, SUCCESS);
        }
        ...
        public String[] getDebugText() {
            return generateDebugText("Tool durability is less than minimum allowed$: {0}%<{1}%", "Tool meets minimum " +
             "durability percentage requirement");
        }
        ...
        @Override
        public void openGlobalModifyMenu(MenuGlobalConfiguration parent) {
            parent.open(new MenuModifyFloat(parent, parent.plugin, parent.player, name, 0f, 1f, true, globalValue,
             (value) -> {
                globalValue = value;
            }));
        }
        ...
        @Override
        public void openToolModifyMenu(MenuToolConfiguration parent, Tool tool) {
            parent.open(new MenuModifyFloat(parent, parent.plugin, parent.player, name, 0f, 1f, true,
toolValues.get(tool), (value) -> {
                if (value == null) toolValues.remove(tool);
                else toolValues.put(tool, value);
            }));
    ...
    public static Option<Float> MAX_DURABILITY_PERCENT = new Option<Float>("Max Durability Percent", true, true, true
    , null) {
        @Override
        public Float load(Object o) {
            return loadFloat(o);
...
Math.round(treeValues.get(tree) * 1000) / 10f);
        ...
        @Override
        public String[] getDebugText() {
            return generateDebugText("Tool durability is greater than maximum allowed: {0}%>{1}%", "Tool meets " +
             "maximum durability percentage requirement");
        }
 ...
 treeValues.get(tree), (value) -> {
    ...
    public static Option<ArrayList<String>> REQUIRED_LORE = new Option<ArrayList<String>>("Required Lore", true, true
    , true, null) {
        @Override
        ...
        @Override
        public void openGlobalModifyMenu(MenuGlobalConfiguration parent) {
            parent.open(new MenuModifyStringList(parent, parent.plugin, parent.player, name, true, globalValue,
(value) -> {
                globalValue = value;
            }));
        }
        ...
        @Override
        public void openToolModifyMenu(MenuToolConfiguration parent, Tool tool) {
            parent.open(new MenuModifyStringList(parent, parent.plugin, parent.player, name, true,
             toolValues.get(tool), (value) -> {
                if (value == null) toolValues.remove(tool);
                else toolValues.put(tool, value);
            }));
    ...
    public static Option<String> REQUIRED_NAME = new Option<String>("Required Name", true, true, true, null) {
        ...
        @Override
        public String[] getDebugText() {
            return generateDebugText("Tool name {0} " + ChatColor.RESET + "does not match required name$: {1}", "Tool" +
             " name matches");
        }
        ...
        @Override
        public void openToolModifyMenu(MenuToolConfiguration parent, Tool tool) {
            parent.openAnvilGUI(toolValues.containsKey(tool) ? toolValues.get(tool) : "", "Edit Required Name", (p,
        str) -> {
        ...
        @Override
        public void openTreeModifyMenu(MenuTreeConfiguration parent, Tree tree) {
            parent.openAnvilGUI(treeValues.containsKey(tree) ? treeValues.get(tree) : "", "Edit Required Name", (p,
             str) -> {
                String value = str == null || str.isEmpty() || str.equalsIgnoreCase("null") ? null : str;
                if (value == null) treeValues.remove(tree);
                else treeValues.put(tree, value);
    ...
    public static Option<HashSet<String>> REQUIRED_PERMISSIONS = new Option<HashSet<String>>("Required Permissions",
     true, true, true, new HashSet<>(), null) {
        @Override
        ...
        @Override
        public String[] getDebugText() {
            return generateDebugText("Player is missing required permission$: {0}", "Player has all required " +
             "permissions");
        }
        ...
        @Override
        public void openGlobalModifyMenu(MenuGlobalConfiguration parent) {
            parent.open(new MenuModifyStringSet(parent, parent.plugin, parent.player, name, true, globalValue,
             (value) -> {
                globalValue = value;
            }));
        }
        ...
        @Override
        public void openToolModifyMenu(MenuToolConfiguration parent, Tool tool) {
            parent.open(new MenuModifyStringSet(parent, parent.plugin, parent.player, name, true,
toolValues.get(tool), (value) -> {
                if (value == null) toolValues.remove(tool);
                else toolValues.put(tool, value);
            }));
    ...
    public static Option<Integer> MIN_TIME = new Option<Integer>("Min Time", true, true, true, null) {
        ...
        public DebugResult doCheck(TreeFeller plugin, Tool tool, Tree tree, Block block, Player player, ItemStack axe) {
            ...
            if (axe.getType().getMaxDurability() > 0) {
                ...
                if (toolValues.containsKey(tool)) {
                    if (dayTime < toolValues.get(tool))
                        return new DebugResult(this, TOOL, dayTime, toolValues.get(tool));
                }
                if (treeValues.containsKey(tree)) {
                    if (dayTime < treeValues.get(tree))
                        return new DebugResult(this, TREE, dayTime, treeValues.get(tree));
                }
            }
            return new DebugResult(this, SUCCESS);
        }
        ...
        @Override
        public void openGlobalModifyMenu(MenuGlobalConfiguration parent) {
            parent.open(new MenuModifyInteger(parent, parent.plugin, parent.player, name, 0, 24000, true, globalValue
            , (value) -> {
                globalValue = value;
            }));
        }
        ...
        @Override
        public void openToolModifyMenu(MenuToolConfiguration parent, Tool tool) {
            parent.open(new MenuModifyInteger(parent, parent.plugin, parent.player, name, 0, 24000, true,
             toolValues.get(tool), (value) -> {
                if (value == null) toolValues.remove(tool);
                else toolValues.put(tool, value);
            }));
    ...
    public static Option<Integer> MAX_TIME = new Option<Integer>("Max Time", true, true, true, null) {
        ...
        @Override
        public String[] getDebugText() {
            return generateDebugText("Time is greater than maximum allowed$: {0}>{1}", "Time meets maximum " +
             "requirement");
        }
 ...
 treeValues.get(tree), (value) -> {
    ...
    public static Option<Integer> MIN_PHASE = new Option<Integer>("Min Phase", true, true, true, null) {
        ...
        @Override
        public void openGlobalModifyMenu(MenuGlobalConfiguration parent) {
            parent.open(new MenuModifyInteger(parent, parent.plugin, parent.player, name, 0, 7, true, globalValue,
                 (value) -> {
                globalValue = value;
            }));
        }
        ...
        @Override
        public void openToolModifyMenu(MenuToolConfiguration parent, Tool tool) {
            parent.open(new MenuModifyInteger(parent, parent.plugin, parent.player, name, 0, 7, true,
 toolValues.get(tool), (value) -> {
                if (value == null) toolValues.remove(tool);
                else toolValues.put(tool, value);
            }));
    ...
    public static Option<Integer> MAX_PHASE = new Option<Integer>("Max Phase", true, true, true, null) {
        ...
        @Override
        public String[] getDebugText() {
            return generateDebugText("Phase is greater than maximum allowed$: {0}>{1}", "Phase meets maximum " +
             "requirement");
        }
    ...
    public static Option<Integer> MIN_FOOD = new Option<Integer>("Min Food", true, true, true, null) {
        ...
        @Override
        public void openGlobalModifyMenu(MenuGlobalConfiguration parent) {
            parent.open(new MenuModifyInteger(parent, parent.plugin, parent.player, name, 0, 20, true, globalValue,
             (value) -> {
                globalValue = value;
            }));
        }
        ...
        @Override
        public void openToolModifyMenu(MenuToolConfiguration parent, Tool tool) {
            parent.open(new MenuModifyInteger(parent, parent.plugin, parent.player, name, 0, 20, true,
             toolValues.get(tool), (value) -> {
                if (value == null) toolValues.remove(tool);
                else toolValues.put(tool, value);
            }));
...
treeValues.get(tree), (value) -> {
    ...
    public static Option<Integer> CUSTOM_MODEL_DATA = new Option<Integer>("Custom Model Data", true, true, true, null) {
        ...
        @Override
        public void openGlobalModifyMenu(MenuGlobalConfiguration parent) {
            parent.open(new MenuModifyInteger(parent, parent.plugin, parent.player, name, Integer.MIN_VALUE,
                    Integer.MAX_VALUE, true, globalValue, (value) -> {
                globalValue = value;
            }));
        }
    ...
    public static Option<ArrayList<Tree>> ALLOWED_TREES = new Option<ArrayList<Tree>>("Allowed Trees", false, true,
     false, null) {
        @Override
        ...
        @Override
        public String getDesc(boolean ingame) {
            return "If set, the tool can only fell specific trees. The given value is a list of tree indicies, " +
             "starting at 0 (the first tree defined is 0, the second is 1, etc.)";
        }
        ...
        @Override
        public void openGlobalModifyMenu(MenuGlobalConfiguration parent) {
            parent.open(new MenuModifyTreeSet(parent, parent.plugin, parent.player, name, true, globalValue,
             (value) -> {
        ...
        @Override
        public void openToolModifyMenu(MenuToolConfiguration parent, Tool tool) {
            parent.open(new MenuModifyTreeSet(parent, parent.plugin, parent.player, name, true, toolValues.get(tool),
            (value) -> {
                if (value == null) toolValues.remove(tool);
        ...
        @Override
        public void openTreeModifyMenu(MenuTreeConfiguration parent, Tree tree) {
            parent.open(new MenuModifyTreeSet(parent, parent.plugin, parent.player, name, true, treeValues.get(tree),
                    (value) -> {
                if (value == null) treeValues.remove(tree);
    ...
    public static OptionBoolean ENABLE_ADVENTURE = new OptionBoolean("Enable Adventure", true, true, true, false) {
        ...
        public DebugResult doCheck(TreeFeller plugin, Tool tool, Tree tree, Block block, Player player, ItemStack axe) {
            ...
            if (toolValues.get(tool) == null && treeValues.get(tree) == null && Objects.equals(globalValue, false))
                return new DebugResult(this, GLOBAL);
        ...
        @Override
        public String[] getDebugText() {
            return generateDebugText("TreeFeller is disabled in adventure mode", "Tool is disabled in adventure mode"
            , "Tree is disabled in adventure mode", "All components OK for adventure mode");
        }
    ...
    public static OptionBoolean ENABLE_SURVIVAL = new OptionBoolean("Enable Survival", true, true, true, true) {
        ...
        @Override
        public String[] getDebugText() {
            return generateDebugText("TreeFeller is disabled in survival mode", "Tool is disabled in survival mode",
            "Tree is disabled in survival mode", "All components OK for survival mode");
        }
    ...
    public static OptionBoolean ENABLE_CREATIVE = new OptionBoolean("Enable Creative", true, true, true, false) {
        ...
        @Override
        public String[] getDebugText() {
            return generateDebugText("TreeFeller is disabled in creative mode", "Tool is disabled in creative mode",
        "Tree is disabled in creative mode", "All components OK for creative mode");
        }
    ...
    public static OptionBoolean WITH_SNEAK = new OptionBoolean("With Sneak", true, true, true, false) {
        ...
        @Override
        public String[] getDebugText() {
            return generateDebugText("TreeFeller is disabled when sneaking", "Tool is disabled when sneaking", "Tree " +
             "is disabled when sneaking", "Felling allowed when sneaking");
        }
    ...
    public static OptionBoolean WITHOUT_SNEAK = new OptionBoolean("Without Sneak", true, true, true, true) {
        ...
        @Override
        public String[] getDebugText() {
            return generateDebugText("TreeFeller is disabled when not sneaking", "Tool is disabled when not sneaking"
            , "Tree is disabled when not sneaking", "Felling allowed when not sneaking");
        }
        ...
        public DebugResult doCheck(TreeFeller plugin, Tool tool, Tree tree, Block block, Player player, ItemStack axe) {
            if (toolValues.containsKey(tool)) {
                ...
                for (String world : worlds) {
                    if (world.equalsIgnoreCase(block.getWorld().getName()) || world.equalsIgnoreCase(block.getWorld().getUID().toString())) {
                        if (blacklist) {
                            return new DebugResult(this, TOOL,
                    block.getWorld().getName() + " (" + block.getWorld().getUID() + ")");
                        } else {
                            foundWorld = true;
                            break;
                        }
            ...
            if (treeValues.containsKey(tree)) {
                ...
                for (String world : worlds) {
                    if (world.equalsIgnoreCase(block.getWorld().getName()) || world.equalsIgnoreCase(block.getWorld().getUID().toString())) {
                        if (blacklist) {
                            return new DebugResult(this, TREE,
                             block.getWorld().getName() + " (" + block.getWorld().getUID() + ")");
                        } else {
                            foundWorld = true;
                            break;
            ...
            if (cooldown != null) {
                ...
                if (globalValue != null) {
                    ...
                    if (diff / 50 <= globalValue) {
                        return new DebugResult(this, GLOBAL, (globalValue - diff), (globalValue - diff) / 50,
(globalValue - diff) / 1000);
                    }
                }
                if (toolValues.containsKey(tool) && cooldown.toolCooldowns.containsKey(tool)) {
                    ...
                    if (diff / 50 <= toolValues.get(tool)) {
                        return new DebugResult(this, TOOL, (toolValues.get(tool) - diff),
 (toolValues.get(tool) - diff) / 50, (toolValues.get(tool) - diff) / 1000);
                    }
                }
                if (treeValues.containsKey(tree) && cooldown.treeCooldowns.containsKey(tree)) {
                    ...
                    if (diff / 50 <= treeValues.get(tree)) {
                        return new DebugResult(this, TREE, (treeValues.get(tree) - diff),
                         (treeValues.get(tree) - diff) / 50, (treeValues.get(tree) - diff) / 1000);
                    }
                }
            }
            return new DebugResult(this, SUCCESS);
        ...
        @Override
        public String[] getDebugText() {
            return generateDebugText("Cooldown remaining: {0}ms", "Tool cooldown remaining: {0}ms", "Tree cooldown " +
 "remaining: {0}ms", "Cooldown ready");
        }
    ...
    public static OptionBoolean CASCADE = new OptionBoolean("Cascade", true, true, true, false) {
        @Override
        public String getDesc(boolean ingame) {
            return "If enabled, connected trees will also be felled resulting in a cascade that could fell an entire " +
             "forest";
        }
    ...
    public static Option<Integer> PARALLEL_CASCADE_LIMIT = new Option<Integer>("Parallel Cascade Limit", true, false,
                false, 1) {
        @Override
        public Integer load(Object o) {
            return loadInt(o);
 ...
 treeValues.get(tree), (value) -> {
    ...
    public static Option<Integer> CASCADE_CHECK_LIMIT = new Option<Integer>("Cascade Check Limit", true, false, false
    , 64) {
        @Override
        public Integer load(Object o) {
            return loadInt(o);
        ...
        @Override
        public String getDesc(boolean ingame) {
            return "How many cascade checks can happen in the same tick? (Much less performance impact, but will help" +
             " performance when cutting down big trees)";
        }
    ...
    public static Option<ArrayList<Tree>> CASCADE_TREES = new Option<ArrayList<Tree>>("Cascade Trees", false, false,
        true, null) {
        @Override
        public ArrayList<Tree> load(Object o) {
            if (o instanceof Iterable) {
        ...
        @Override
        public String getDesc(boolean ingame) {
            return "Which trees should be checked during a cascade? (If not set, only the tree which was originally " +
             "cut will be checked) The given value is a list of tree indicies, starting at 0 (the first tree defined " +
 ...
 Object defaultConfigValue) {
    ...
    public static String[] generateDebugText(String globalWith$, String success) {
        return new String[]{globalWith$.replace("$", ""), globalWith$.replace("$", " for tool"), globalWith$.replace(
                "$", " for tree"), success};
    }
    ...
    public ArrayList<String> getDescription(boolean ingame) {
        ...
        if (s.contains("\n")) {
            Collections.addAll(description, s.split("\n"));
     ...
     * This object is the result of <code>getConfig().get(getGlobalName())</code> for global options, or <code>list
            * .get(getLocalName()) for local options</code>
     *
    ...
    public final DebugResult checkTrunk(TreeFeller plugin, Tool tool, Tree tree,
     HashMap<Integer, ArrayList<Block>> blocks, Block block) {
        if (globalValue == null && !treeValues.containsKey(tree) && !toolValues.containsKey(tool)) return null;
        return doCheckTrunk(plugin, tool, tree, blocks, block);
    }

    public final DebugResult checkTree(TreeFeller plugin, Tool tool, Tree tree,
 HashMap<Integer, ArrayList<Block>> blocks, int leaves) {
        if (globalValue == null && !treeValues.containsKey(tree) && !toolValues.containsKey(tool)) return null;
    ...
    protected DebugResult doCheckTrunk(TreeFeller plugin, Tool tool, Tree tree,
     HashMap<Integer, ArrayList<Block>> blocks, Block block) {
        return null;
    }
