...
public class TreeFeller extends JavaPlugin {
    ...
    private static HashMap<Integer, ArrayList<Block>> getBlocks(Collection<Material> materialTypes,
                                                                Block startingBlock, int maxDistance, int maxBlocks,
                                                                boolean diagonal, boolean playerLeaves,
                                                                boolean ignoreLeafData) {
        return getBlocks(materialTypes, startingBlock, maxDistance, maxBlocks, diagonal, playerLeaves, ignoreLeafData
                , false);
    }
    ...
    private static HashMap<Integer, ArrayList<Block>> getBlocks(Collection<Material> materialTypes,
                                                                Block startingBlock, int maxDistance, int maxBlocks,
                                                                boolean diagonal, boolean playerLeaves,
                                                                boolean ignoreLeafData, boolean invertLeafDirection) {
        //layer zero
                    ...
                    for (int x = -1; x <= 1; x++) {
                        for (int y = -1; y <= 1; y++) {
                            for (int z = -1; z <= 1; z++) {
                                ...
                                if (newBlock.getBlockData() instanceof Leaves newLeaf) {
                                    if (!playerLeaves && newLeaf.isPersistent()) continue;
                                    if (!ignoreLeafData) {
                                        if (block.getBlockData() instanceof Leaves oldLeaf) {
     ...
     * @return the items that would have been dropped. <b>This is not the actual dropped items, but possible dropped
     * items</b> Returns null if the tree was not felled.
     */
    public ArrayList<ItemStack> fellTree(Block block, Player player, ItemStack axe, boolean dropItems) {
        ...
        if (player != null && player.getGameMode() != GameMode.CREATIVE && player.getGameMode() != GameMode.SPECTATOR) {
            ...
            double consumeFood = Option.CONSUMED_FOOD_BASE.get(tool, tree) + Option.CONSUMED_FOOD_LOGS.get(tool,
                    tree) * logCount + Option.CONSUMED_FOOD_LEAVES.get(tool, tree) * leafCount;
            double consumeHealth = Option.CONSUMED_HEALTH_BASE.get(tool, tree) + Option.CONSUMED_HEALTH_LOGS.get(tool
                    , tree) * logCount + Option.CONSUMED_HEALTH_LEAVES.get(tool, tree) * leafCount;
            if (consumeFood > 0) {
     ...
     * @param axe    The ItemStack to use while detecting the tree; can be null. (Used to find the Tool; tool
     *               durability for partial trees is not considered)
     ...
     * @param axe       The ItemStack to use while detecting the tree; can be null. (Used to find the Tool; tool
     *                  durability for partial trees is not considered)
     * @param checkFunc A function to perform any additional checks to ensure a tree is valid. returns true if the
     *                  tree is valid, null if the tool is invalid, or false if the tree is invalid.
     * @return a DetectedTree object, or null if no tree was found
     */
    public DetectedTree detectTree(Block block, Player player, ItemStack axe,
        ...
        for (Tree tree : trees) {
            if (!tree.trunk.contains(material)) {
                ...
                if (roots != null && roots.contains(material)) {
                    Block b = getNearest(block, tree.trunk, roots, Option.ROOT_DISTANCE.get(null, tree), true,
                            Option.PLAYER_LEAVES.get(null, tree));
            ...
            for (Tool tool : tools) {
                ...
                HashMap<Integer, ArrayList<Block>> blocks = getBlocks(tree.trunk, block, scanDistance,
                        maxBlocks == null ? Integer.MAX_VALUE : (maxBlocks * 2), true, false, false);//TODO what if
                // the trunk is made of leaves?
                for (Option o : Option.options) {
                ...
                for (int i : distances) {
                    for (Block b : blocks.get(i)) {
                        HashMap<Integer, ArrayList<Block>> someLeaves = getBlocksWithLeafCheck(tree.trunk,
                                tree.leaves, b, Option.LEAF_DETECT_RANGE.get(tool, tree),
                                Option.DIAGONAL_LEAVES.get(tool, tree), Option.PLAYER_LEAVES.get(tool, tree),
                                Option.IGNORE_LEAF_DATA.get(tool, tree), Option.FORCE_DISTANCE_CHECK.get(tool, tree));
                ...
                if (Option.SAPLING.get(tool, tree) != null && Option.REPLANT_SAPLINGS.get(tool, tree)) {
                    ...
                    for (Block b : possibleSaplings.keySet()) {
                        detected.addSapling(Option.USE_INVENTORY_SAPLINGS.get(tool, tree) ? player : null, b,
                                Option.SAPLING.get(tool, tree));
                    }
                }
                return detected;
            }
    ...
    private HashMap<Integer, ArrayList<Block>> getBlocksWithLeafCheck(ArrayList<Material> trunk,
                                                                      ArrayList<Material> leaves, Block startingBlock
            , int maxDistance, boolean diagonal, boolean playerLeaves, boolean ignoreLeafData,
                                                                      boolean forceDistanceCheck) {
    ...
    private Block getNearest(Block from, ArrayList<Material> to, Collection<Material> materialTypes, int max,
                             boolean diagonal, boolean playerLeaves) {
    ...
    public void onEnable() {
        ...
        try {
            ...
            logger.log(Level.WARNING, "Failed to add permissions! Did you reload the plugin? (If you just want to " +
                    "reload the config, use /treefeller reload)");
        }
    ...
    public void reload() {
        ...
        if (effects != null) {
            for (Object o : effects) {
                if (o instanceof LinkedHashMap map) {
                    ...
                    logger.log(Level.INFO, "Unknown tool declaration: {0} | {1}", new Object[]{o.getClass().getName()
                            , o.toString()});
                }
            }
        }
        ...
        if (Option.STARTUP_LOGS.isTrue()) {
            logger.log(Level.INFO, "Server version: {0}", Bukkit.getServer().getClass().getPackage().getName().split(
                    "\\.")[3].substring(1));
        ...
        for (Object o : trees) {
            if (o instanceof ArrayList) {
                ...
                Material l =
                        Material.matchMaterial(((String) o).replace("STRIPPED_", "").replace("LOG", "LEAVES").replace("WOOD", "LEAVES"));
                if (t != null) trunk.add(t);
                if (l != null) leaves.add(l);
                if (trunk.isEmpty() || leaves.isEmpty()) {
                    logger.log(Level.WARNING, "Cannot load tree: {0}", o);
        ...
        for (Object o : tools) {
            if (o instanceof LinkedHashMap map) {
                if (!map.containsKey("type") || !(map.get("type") instanceof String typ)) {
    ...
    private void breakBlock(DetectedTree detectedTree, boolean dropItems, Tree tree, Tool tool, ItemStack axe,
        ...
        //        double dropChance = dropItems?(isLeaf?Option.LEAF_DROP_CHANCE.get(tool, tree):Option
        //        .LOG_DROP_CHANCE.get(tool, tree)):0;
    ...
    private void cascade(DetectedTree detectedTree, boolean dropItems, Tree tree, Tool tool, ItemStack axe,
            ...
            cascadeTask = new BukkitRunnable() {
                ...
                public void run() {
                    ...
                    while (checks < maxChecks && cascades < maxCascades && !pendingCascades.isEmpty()) {
                        ...
                        BlockFace[] directions = new BlockFace[]{BlockFace.NORTH, BlockFace.EAST, BlockFace.SOUTH,
                                BlockFace.WEST, BlockFace.UP, BlockFace.DOWN};
                        for (BlockFace dir : directions) {
                            Block start = cascade.block.getRelative(dir);
                            if (prevLeaves.contains(start) || prevTrunks.contains(start)) continue;
                            ArrayList<Block> detectedLeaves = toList(getBlocks(new ArrayList<>(allLeaves), start,
                                    maxRange, 64, diagonal, player, ignoreData, true));
    ...
    Collection<? extends ItemStack> getDropsWithBonus(Block block, Tool tool, Tree tree, ItemStack axe, int[] xp,
        ...
        double dropChance = tree.trunk.contains(block.getType()) ? Option.LOG_DROP_CHANCE.get(tool, tree) :
                Option.LEAF_DROP_CHANCE.get(tool, tree);
    ...
    Collection<? extends ItemStack> getDrops(Block block, Tool tool, Tree tree, ItemStack axe, int[] xp) {
        ...
        if (fortune && axe.containsEnchantment(Enchantment.LOOT_BONUS_BLOCKS))
    ...
    public void toggle(Player player, boolean state) {
        boolean inverted = Option.DEFAULT_ENABLED.isTrue();
        boolean shouldBeToggled = inverted != state;
    ...
    private void refreshPatronsList() {
        try {
            ...
            file.delete();
            downloadFile("https://raw.githubusercontent.com/ThizThizzyDizzy/nc-reactor-generator/overhaul/patrons" +
                    ".txt", file.getAbsoluteFile());
        ...
        }//don't crash if the patrons list fails to download
    }
