...
public class CoreProtectCompat extends InternalCompatibility {
    ...
    public void addBlock(Player player, Block block, BlockState was) {
        net.coreprotect.CoreProtect.getInstance().getAPI().logPlacement(player == null ? null : player.getName(),
                block.getLocation(), block.getType(), block.getBlockData());
    }

    @Override
    public void removeBlock(Player player, Block block) {
        net.coreprotect.CoreProtect.getInstance().getAPI().logRemoval(player == null ? null : player.getName(),
                block.getLocation(), block.getType(), block.getBlockData());
    }
