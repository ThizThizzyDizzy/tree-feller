...
public class TreeFellerCompat {
    ...
    public static void breakBlock(Tree tree, Tool tool, Player player, ItemStack axe, Block block,
    ...
    public static void fellTree(Block block, Player player, ItemStack axe, Tool tool, Tree tree, HashMap<Integer,
        ...
        for (PluginCompatibility compat : compatibilities) {
            if (compat.getPluginName().equalsIgnoreCase(compatibility.getPluginName())) {
                if (compat instanceof InternalCompatibility) {
                    Bukkit.getLogger().log(Level.WARNING, "Overriding internal compatibility for {0}!",
                            compat.getPluginName());
                    override = compat;
                    break;
                } else {
                    Bukkit.getLogger().log(Level.SEVERE, "External compatibility already exists for {0}! Ignoring..."
                            , compat.getPluginName());
                    return;
                }
            }
