...
public class WorldGuardCompat2 {
    ...
    public static Block test(Player player, Iterable<Block> blocks) {
        ...
        boolean canBypass =
                com.sk89q.worldguard.WorldGuard.getInstance().getPlatform().getSessionManager().hasBypass(lp,
                        lp.getWorld());
        if (!canBypass) {
            ...
            if (container == null) {//RegionContainer is null. I don't know what this means, probably that there's no
                // regions?
                return null;//all is good, no regions(?)
            }
            com.sk89q.worldguard.protection.regions.RegionQuery query = container.createQuery();
            for (Block b : blocks) {
                com.sk89q.worldedit.util.Location loc = new com.sk89q.worldedit.util.Location(lp.getWorld(), b.getX()
                        , b.getY(), b.getZ());
                ...
                if (query.queryState(loc, lp, com.sk89q.worldguard.protection.flags.Flags.BLOCK_BREAK) == com.sk89q.worldguard.protection.flags.StateFlag.State.DENY)
                    return b;
