...
public class Message {
    ...
    static {
        ...
        addMessage("prevent-breakage", "Felling this tree would break the tool",
                new ItemBuilder(Material.WOODEN_AXE).setDurability(1));
        ...
        addMessage("durability-low", "Tool durability is too low: {0}<{1}",
                new ItemBuilder(Material.GOLDEN_AXE).setDurability(10));
    ...
    private static void addMessage(Option option) {
        ...
        if (option.tool)
            addMessage(option.getGlobalName() + "-tool", option.getToolDebugText(),
                    option.getConfigurationDisplayItem());
        if (option.tree)
            addMessage(option.getGlobalName() + "-tree", option.getTreeDebugText(),
                    option.getConfigurationDisplayItem());
