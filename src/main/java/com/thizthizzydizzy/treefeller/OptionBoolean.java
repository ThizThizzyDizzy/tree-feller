...
public abstract class OptionBoolean extends Option<Boolean> {
    ...
    public Boolean get(Tool tool, Tree tree) {
        if (toolValues.containsKey(tool) || treeValues.containsKey(tree))
            return Objects.equals(toolValues.get(tool), true) || Objects.equals(treeValues.get(tree), true);
    ...
    public void openToolModifyMenu(MenuToolConfiguration parent, Tool tool) {
        parent.open(new MenuModifyBoolean(parent, parent.plugin, parent.player, name, true, toolValues.get(tool),
                (value) -> {
    ...
    public void openTreeModifyMenu(MenuTreeConfiguration parent, Tree tree) {
        parent.open(new MenuModifyBoolean(parent, parent.plugin, parent.player, name, true, treeValues.get(tree),
                (value) -> {
