...
public enum DirectionalFallBehavior {
    RANDOM(Material.BELL, "The tree will fall in a random direction") {
        @Override
        Vector getDefaultDirectionalVel(long seed, Player player, Block block, boolean lockCardinal,
                                        double directionalFallVelocity) {
    ...
    TOWARD(Material.DISPENSER, "The tree will fall towards the player") {
        ...
        Vector getDefaultDirectionalVel(long seed, Player player, Block block, boolean lockCardinal,
                                        double directionalFallVelocity) {
            return player == null ? null : new Vector(player.getLocation().getX() - block.getLocation().getX(),
    ...
    AWAY(Material.DROPPER, "The tree will fall away from the player") {
        ...
        Vector getDefaultDirectionalVel(long seed, Player player, Block block, boolean lockCardinal,
            ...
            return player == null ? null : new Vector(player.getLocation().getX() - block.getLocation().getX(),
                    player.getLocation().getY() - block.getLocation().getY(),
                    player.getLocation().getZ() - block.getLocation().getZ()).multiply(-1);
        }
    },
    LEFT(Material.CROSSBOW, "The tree will fall to the player's left") {
        ...
        Vector getDefaultDirectionalVel(long seed, Player player, Block block, boolean lockCardinal,
                                        double directionalFallVelocity) {
            return player == null ? null : new Vector(-(player.getLocation().getZ() - block.getLocation().getZ()),
                    player.getLocation().getY() - block.getLocation().getY(),
    ...
    RIGHT(Material.WOODEN_SWORD, "The tree will fall to the player's right") {
        ...
        Vector getDefaultDirectionalVel(long seed, Player player, Block block, boolean lockCardinal,
            ...
            return player == null ? null : new Vector(-(player.getLocation().getZ() - block.getLocation().getZ()),
                    player.getLocation().getY() - block.getLocation().getY(),
                    player.getLocation().getX() - block.getLocation().getX()).multiply(-1);
        }
    },
    ...
    DirectionalFallBehavior(Material item, String description) {
        this.item = item;
    ...
    abstract Vector getDefaultDirectionalVel(long seed, Player player, Block block, boolean lockCardinal,
                                             double directionalFallVelocity);

    public Vector getDirectionalVel(long seed, Player player, Block block, boolean lockCardinal,
                                    double directionalFallVelocity) {
