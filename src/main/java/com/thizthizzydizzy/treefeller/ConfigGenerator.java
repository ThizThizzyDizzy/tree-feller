...
public class ConfigGenerator {
    ...
    public static void generateConfiguration(TreeFeller plugin) {
        ...
        add("# AIR can be used instead of an item name if you want every item, including an empty hand, to fell a " +
                "tree");
        add("# Examples of valid tools:");
        add("#    - {type: WOODEN_AXE, required-enchantments: {efficiency: 4, unbreaking: 1}, banned-enchantments: " +
                "{unbreaking: 2}, min-durability: 4, max-durability-percent: 0.9}");
        add("#    - {type: DIAMOND_AXE, required-enchantments: {efficiency: 4}, banned-enchantments: {unbreaking: 2}," +
                " required-lore: [A line of lore, Another line of lore]}");
        add("#    - {type: GOLDEN_AXE, required-permissions: [treefeller.example,treefeller.anotherexample]}");
        add("#    - STONE_AXE");
        add("#    - {type: IRON_AXE, allowed-trees: [0,1,3]} <-this tool can only cut down the first, second, and " +
                "fourth trees defined.");
        ...
        add("#   sound     Any sound");
        add("#   explosion An explosion that Will destroy blocks and items- This will occur after the block is " +
                "destroyed");
        ...
        add("# Marker settings:");
        add("#   permanent: (true|false) If true, an armor stand will be created. If false, an area effect cloud will" +
                " be created. (Area effect clouds last exactly 1 tick)");
        add("#   tags: [<values>]   A list of tags to apply to the created entity. (Note that the \"tree_feller\" tag" +
                " is always applied)");
        add("# Examples of valid effects:");
        add("#   - {name: smoke, chance: 1, location: logs, type: particle, particle: smoke, dx: 0.5, dy: 0.5, dz: 0" +
                ".5, speed: .01, count: 10}");
        add("#   - {name: explosion, chance: .01, location: tool, type: explosion, power: 4}");
        add("#   - {name: ghost sound, chance: .1, location: tree, type: sound, sound: ambient.cave, volume: 10, " +
                "pitch: 0.5}");
        add("effects:");
        ...
        add();
        add("# Here, you can customize what messages are sent the tree feller is unable to cut down a tree. Most " +
                "options are customizable; The debug messages are provided for reference");
        ...
        add("# -success     This defines what message is sent when global, tree, and tool requirements for are all " +
                "met");
        ...
        add("# protected        This is sent when a tree cannot be felled due to a protection plugin (This may be on " +
                "top of that plugin's protected message)");
    ...
    public static void main(String[] args) {
        if (args.length == 1 && args[0].equals("genConfig")) {
            ...
            add("# AIR can be used instead of an item name if you want every item, including an empty hand, to fell a" +
                    " tree");
            add("# Examples of valid tools:");
            add("#    - {type: WOODEN_AXE, required-enchantments: {efficiency: 4, unbreaking: 1}, " +
                    "banned-enchantments: {unbreaking: 2}, min-durability: 4, max-durability-percent: 0.9}");
            add("#    - {type: DIAMOND_AXE, required-enchantments: {efficiency: 4}, banned-enchantments: {unbreaking:" +
                    " 2}, required-lore: [A line of lore, Another line of lore]}");
            add("#    - {type: GOLDEN_AXE, required-permissions: [treefeller.example,treefeller.anotherexample]}");
            add("#    - STONE_AXE");
            add("#    - {type: IRON_AXE, allowed-trees: [0,1,3]} <-this tool can only cut down the first, second, and" +
                    " fourth trees defined.");
            ...
            add("    - [[OAK_LOG, OAK_WOOD], [AZALEA_LEAVES, FLOWERING_AZALEA_LEAVES], {sapling: [AZALEA, " +
                    "FLOWERING_AZALEA], max-saplings: 1, diagonal-leaves: true}]");
            add("    - [[MANGROVE_LOG, MANGROVE_WOOD], [MANGROVE_ROOTS, MANGROVE_LEAVES], {roots: [MANGROVE_ROOTS], " +
                    "sapling: [MANGROVE_PROPAGULE], max-saplings: 1, max-trunks: 16, " +
                    "max-horizontal-trunk-pillar-length: 16, leaf-detect-range: 16, leaf-break-range: 16, " +
                    "required-logs: 3, root-distance: 16, diagonal-leaves: true}]");
            add("    - [[CRIMSON_STEM, CRIMSON_HYPHAE], [NETHER_WART_BLOCK, SHROOMLIGHT], {sapling: CRIMSON_FUNGUS, " +
                    "max-saplings: 1, grass: [CRIMSON_NYLIUM], diagonal-leaves: true, leaf-detect-range: 8, " +
                    "leaf-break-range: 8}]");
            add("    - [[WARPED_STEM, WARPED_HYPHAE], [WARPED_WART_BLOCK, SHROOMLIGHT], {sapling: WARPED_FUNGUS, " +
                    "max-saplings: 1, grass: [WARPED_NYLIUM], diagonal-leaves: true, leaf-detect-range: 8, " +
                    "leaf-break-range: 8}]");
            add();
            add("# Here, you can create custom effects for trees or tools");
            add("# Fields:");
            ...
            add("#   fire: (true|false) Weather or not to light fires with the explosion (Default false)");
            add("# Marker settings:");
            add("#   permanent: (true|false) If true, an armor stand will be created. If false, an area effect cloud " +
                    "will be created. (Area effect clouds last exactly 1 tick)");
            add("#   tags: [<values>]   A list of tags to apply to the created entity. (Note that the \"tree_feller\"" +
                    " tag is always applied)");
            add("# Examples of valid effects:");
            add("#   - {name: smoke, chance: 1, location: logs, type: particle, particle: smoke, dx: 0.5, dy: 0.5, " +
                    "dz: 0.5, speed: .01, count: 10}");
            add("#   - {name: explosion, chance: .01, location: tool, type: explosion, power: 4}");
            add("#   - {name: ghost sound, chance: .1, location: tree, type: sound, sound: ambient.cave, volume: 10, " +
                    "pitch: 0.5}");
            add("effects:");
            ...
            add("# -tool        This defines what message is sent when a tool requirement is not met");
            add("# -tree        This defines what message is sent when a tree requirement is not met");
            add("# -success     This defines what message is sent when global, tree, and tool requirements for are " +
                    "all met");
            add("# In addition to the options, there are a few additional messages that can be customized:");
            add("# toggle           This is sent when the player cuts down a tree when the tree feller is off");
            add("# checking         This is sent for each tree/tool pair the tree feller checks");
            add("# durability-low   This is sent when the tool's durability is too low to fell the tree");
            add("# partial          This is sent when a tree is being partially cut");
            add("# protected        This is sent when a tree cannot be felled due to a protection plugin (This may be" +
                    " on top of that plugin's protected message)");
            add("# success          This is sent when a tree is successfully felled");
            add("# For example, if you want a player to be sent a message in the chat if the tree is too small:");
            add("# chat-required-logs: The tree's too small!");
            add();
