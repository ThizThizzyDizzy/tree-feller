...
public class Effect {
    ...
    public Effect(String name, EffectLocation location, double chance, Particle particle, double x, double y,
                  double z, double dx, double dy, double dz, double speed, int count, Object extra) {
    ...
    public enum EffectType {
        PARTICLE(Material.CAMPFIRE) {
            ...
            void play(Effect effect, Block block) {
                block.getWorld().spawnParticle(effect.particle, block.getLocation().add(effect.x + .5, effect.y + .5,
                        effect.z + .5), effect.count, effect.dx, effect.dy, effect.dz, effect.speed, effect.extra);
            }
            ...
            Effect loadEffect(String name, EffectLocation location, double chance, LinkedHashMap map) {
                ...
                switch (particle) {
                    case REDSTONE:
                        extra = new Particle.DustOptions(Color.fromRGB(((Number) map.get("r")).intValue(),
                                ((Number) map.get("g")).intValue(), ((Number) map.get("b")).intValue()),
                                ((Number) map.get("size")).floatValue());
                        break;
                    case ITEM_CRACK:
        ...
        SOUND(Material.NOTE_BLOCK) {
            ...
            @Override
            void play(Effect effect, Block block) {
                block.getWorld().playSound(block.getLocation().add(0.5, 0.5, 0.5), effect.sound, SoundCategory.BLOCKS
                        , effect.volume, effect.pitch);
            }
        ...
        MARKER(Material.ARMOR_STAND) {
            ...
            @Override
            void play(Effect effect, Block block) {
                Entity entity = block.getWorld().spawnEntity(block.getLocation().add(.5, .5, .5), effect.permanent ?
            ...
            Effect loadEffect(String name, EffectLocation location, double chance, LinkedHashMap map) {
                ...
                if (map.containsKey("tags")) {
                    ...
                    if (ob instanceof ArrayList) {
                        ...
                        throw new IllegalArgumentException("Unknown marker tags format: " + ob + "! Please use an " +
                                "array or a String!");
                    }
                }
