    ...
    private void refresh() {
        ...
        add(new Button(size - 6, makeItem(Material.GREEN_CONCRETE).setDisplayName("New tree"), (click) -> {
            if (click == ClickType.LEFT) {
                open(new MenuSelectMaterialSet(this, plugin, player, "New Tree Trunk", false, "block",
                        new HashSet<>(), (mat) -> {
                    return mat.isBlock();
                }, (menu, trunk) -> {
                    if (trunk.isEmpty()) return;
                    menu.open(new MenuSelectMaterialSet(this, plugin, player, "New Tree Leaves", false, "block",
                            new HashSet<>(), (mat) -> {
                        return mat.isBlock();
                    }, (menu2, leaves) -> {
        ...
        add(new Button(size - 4, makeItem(Material.GREEN_CONCRETE).setDisplayName("Detect tree").addLore("Creates a " +
                "new tree based on a template tree in the world").addLore("Right click a tree to select it.").addLore("Make sure the tree is not touching any build or other trees").addLore("There may be a lag spike when detecting a tree"), (click) -> {
            if (click == ClickType.LEFT) {
                close();
        ...
        for (int i = 0; i < Math.min(pageMax, TreeFeller.trees.size() - offset); i++) {
            ...
            add(new Button(index, makeItem(t.getDisplayMaterial()).setDisplayName("Tree #" + i).addLore("Shift-left " +
                    "click to move left").addLore(("Shift-right click to move right")), (click) -> {
