...
public class MenuMessageConfiguration extends Menu {
    ...
    private void refresh() {
        ...
        add(new Button(3, makeItem(Material.DEBUG_STICK).setDisplayName("Debug").addLore(message.debug == null ?
                "null" : message.debug), (click) -> {
            if (click == ClickType.LEFT)
                openAnvilGUI(message.debug == null ? "null" : message.debug, "Set Debug: " + message.name, (player,
                                                                                                            str) -> {
                    message.debug = str;
                });
        }));
        add(new Button(4, makeItem(Material.PAPER).setDisplayName("Chat").addLore(message.chat == null ? "null" :
                message.chat), (click) -> {
            if (click == ClickType.LEFT)
                openAnvilGUI(message.chat == null ? "null" : message.chat, "Set Chat: " + message.name,
                        (player, str) -> {
                    message.chat = str.equalsIgnoreCase("null") ? null : str;
                });
        }));
        add(new Button(5,
                makeItem(Material.IRON_SWORD).setDisplayName("Actionbar").addLore(message.actionbar == null ? "null"
                        : message.actionbar), (click) -> {
            if (click == ClickType.LEFT)
                openAnvilGUI(message.actionbar == null ? "null" : message.actionbar, "Set Actionbar: " + message.name
                        , (player, str) -> {
