    ...
    private void refresh() {
        ...
        add(new Button(size - 5, makeItem(tool.material).addLore("Click to modify material"), (click) -> {
            if (click == ClickType.LEFT)
                open(new MenuModifyMaterial(this, plugin, player, "Modify Tool Material", false, "item",
        ...
        for (int i = 0; i < Math.min(pageMax, Option.options.size() - offset); i++) {
            ...
            add(new Button(index,
                    o.getConfigurationDisplayItem(tool).setDisplayName(o.getFriendlyName()).addLore(ChatColor.GRAY + shorten(Objects.toString(o.getValue(tool)), 42)).addLore(shorten(o.getDescription(true), 42)), (click) -> {
