...
public class MenuTreeConfiguration extends Menu {
    ...
    private void refresh() {
        ...
        add(new Button(size - 6,
                makeItem(tree.trunk.isEmpty() ? Material.OAK_LOG : tree.trunk.get(0)).setDisplayName("Click to modify" +
                        " Trunk"), (click) -> {
            if (click == ClickType.LEFT)
                open(new MenuModifyMaterialSet(this, plugin, player, "Modify Tree Trunk", false, "block",
                        new HashSet<>(tree.trunk), (material) -> {
        ...
        add(new Button(size - 4,
                makeItem(tree.leaves.isEmpty() ? Material.OAK_LEAVES : tree.leaves.get(0)).setDisplayName("Click to " +
                        "modify Leaves"), (click) -> {
            if (click == ClickType.LEFT)
                open(new MenuModifyMaterialSet(this, plugin, player, "Modify Tree Leaves", false, "block",
                        new HashSet<>(tree.leaves), (material) -> {
                    return material.isBlock();
                }, (value) -> {
                    tree.leaves.clear();
        ...
        for (int i = 0; i < Math.min(pageMax, Option.options.size() - offset); i++) {
            ...
            add(new Button(index,
                    o.getConfigurationDisplayItem(tree).setDisplayName(o.getFriendlyName()).addLore(ChatColor.GRAY + shorten(Objects.toString(o.getValue(tree)), 42)).addLore(shorten(o.getDescription(true), 42)), (click) -> {
