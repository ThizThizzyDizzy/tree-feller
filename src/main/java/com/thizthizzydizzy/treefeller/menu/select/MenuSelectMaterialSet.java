...
public class MenuSelectMaterialSet extends Menu {//TODO make this multi-page allowing more to be visible
    ...
    public MenuSelectMaterialSet(Menu parent, Plugin plugin, Player player, String name, boolean allowNull,
                                 String filterName, HashSet<Material> defaultValue,
                                 Function<Material, Boolean> filter, BiConsumer<MenuSelectMaterialSet,
            HashSet<Material>> setFunc) {
    ...
    public void refresh() {
        components.clear();
        add(new Button(0, makeItem(Material.PAPER).setDisplayName(value == null ? "NULL" :
                (value.size() + " " + filterName + (value.size() == 1 ? "" : "s"))).addLore("Click a " + filterName + " in your inventory to add it to the list.").addLore("Right click any " + filterName + " in the list to remove it.").addLore(allowNull ? "Shift-right click this item to set to NULL" : ("Shift-right click this item to clear all " + filterName + "s")), (click) -> {
        ...
        if (value != null) {
            ...
            for (int i = 0; i < Math.min(51, lst.size()); i++) {
                ...
                add(new Button(i + 1, m.isItem() ? makeItem(m) :
                        (makeItem(Material.PAPER).setDisplayName(m.toString())), (click) -> {
        ...
        add(new Button(size - 1, makeItem(value == null || value.isEmpty() ? Material.BARRIER :
                Material.GREEN_CONCRETE).setDisplayName(value == null || value.isEmpty() ? "Back" : "Select"),
                (click) -> {
