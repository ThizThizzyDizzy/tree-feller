...
public class MenuModifyStringDoubleMap extends Menu {
    ...
    public MenuModifyStringDoubleMap(Menu parent, Plugin plugin, Player player, String name, double min, double max,
                                     boolean allowNull, boolean allowNullDoubles,
                                     HashMap<String, Double> defaultValue, Consumer<HashMap<String, Double>> setFunc) {
    ...
    public void refresh() {
        ...
        if (value != null) {
            ...
            for (int i = 0; i < Math.min(value.size(), allowNull ? 51 : 52); i++) {
                int idx = i;
                add(new Button(i + 1,
                        makeItem(Material.PAPER).setDisplayName(strs.get(i) + " (" + value.get(strs.get(i)) + ")").addLore("Left click to modify").addLore("Right click to remove"), (click) -> {
                    if (click == ClickType.LEFT) {
                        open(new MenuModifyDouble(this, plugin, player, "Modify " + strs.get(idx), min, max,
                                allowNullDoubles, value.get(strs.get(idx)), (value) -> {
