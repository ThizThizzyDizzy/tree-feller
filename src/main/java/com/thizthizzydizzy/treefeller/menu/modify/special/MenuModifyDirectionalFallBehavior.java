...
public class MenuModifyDirectionalFallBehavior extends Menu {//TODO make multi-page
    ...
    public MenuModifyDirectionalFallBehavior(Menu parent, Plugin plugin, Player player, String name,
                                             boolean allowNull, DirectionalFallBehavior defaultValue,
                                             DirectionalFallBehavior[] values,
                                             Consumer<DirectionalFallBehavior> setFunc) {
        super(parent, plugin, player, "Modify Directional Fall Behavior (" + name + ")", getSize(values.length,
                allowNull));
        Label label = add(new Label(0, makeItem(Material.PAPER).setDisplayName(Objects.toString(defaultValue))));
        for (int i = 0; i < values.length; i++) {
            int idx = i;
            add(new Button(i + 1, makeItem(values[i].getItem()).addFlag(ItemFlag.HIDE_ATTRIBUTES).setDisplayName("Set" +
                    " to " + values[i]).addLore(shorten(values[i].getDescription(), 42)), (click) -> {
    ...
    private static int getSize(int count, boolean allowNull) {
        ...
        if (actualCount > 54)
            throw new IllegalArgumentException("MenuModifyDirectionalFallBehavior only supports up to 52 values! " +
                    "(including null)");
