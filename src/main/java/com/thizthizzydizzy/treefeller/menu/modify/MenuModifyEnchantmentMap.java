...
public class MenuModifyEnchantmentMap extends Menu {
    ...
    public MenuModifyEnchantmentMap(Menu parent, Plugin plugin, Player player, String name, boolean allowNull,
                                    HashMap<Enchantment, Integer> defaultValue, Consumer<HashMap<Enchantment,
            Integer>> setFunc) {
    ...
    private static int getSize(boolean allowNull) {
        ...
        if (actualCount > 54)
            throw new IllegalArgumentException("MenuModifyEnchantments only supports up to " + (allowNull ? 51 : 52) + " values!");//TODO fix
    ...
    private void refresh() {
        components.clear();
        ItemBuilder builder = makeItem(Material.PAPER).setDisplayName(value == null ? "null" : value.size() + " " +
                "enchantments:");
        if (value != null) {
        ...
        for (int i = 0; i < Math.min(allowNull ? 51 : 52, Enchantment.values().length); i++) {
            ...
            add(new Button(i + 1,
                    makeItem(Material.ENCHANTED_BOOK).enchant(enchantment, level == null ? 1 : level).setDisplayName(enchantment.toString()).addLore("Left click to increase enchantment level").addLore("Right click to decrease enchantment level").addLore("Current level: " + (level == null ? "null" : level)), (click) -> {
        ...
        if (allowNull) {
            add(new Button(Enchantment.values().length + 1, makeItem(Material.BLACK_CONCRETE).setDisplayName("Set to " +
                    "NULL"), (click) -> {
