...
public class MenuModifyMaterialMaterialMap extends Menu {//TODO make this multi-page allowing more to be visible
    ...
    public MenuModifyMaterialMaterialMap(Menu parent, Plugin plugin, Player player, String name, String keyFilterName
            , Predicate<Material> keyFilter, String valueFilterName, Predicate<Material> valueFilter,
                                         boolean allowNull, HashMap<Material, Material> defaultValue,
                                         Consumer<HashMap<Material, Material>> setFunc) {
    ...
    public void refresh() {
        ...
        if (selected == null) {
            add(new Label(0,
                    makeItem(Material.PAPER).setDisplayName("Click " + an(keyFilterName) + " " + keyFilterName + " in" +
                            " your inventory to select it.")));
        } else {
            add(new Button(0,
                    makeItem(selected).setDisplayName("Click " + an(valueFilterName) + " " + valueFilterName + " in " +
                            "your inventory to add them to the list.").addLore("Right click to clear selection"),
                    (click) -> {
        ...
        if (value != null) {
            ...
            for (int i = 0; i < Math.min(value.size(), allowNull ? 51 : 52); i++) {
                int idx = i;
                add(new Button(i + 1,
                        makeItem(materials.get(i)).addLore("Value: " + value.get(materials.get(i)).toString()).addLore("Right click to remove"), (click) -> {
                    if (click == ClickType.RIGHT) {
        ...
        if (allowNull) {
            add(new Button(value == null ? 1 : (value.size() + 1), makeItem(Material.BLACK_CONCRETE).setDisplayName(
                    "Set to NULL"), (click) -> {
