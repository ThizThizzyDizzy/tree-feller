...
public class MenuModifySpawnSaplings extends Menu {
    ...
    public MenuModifySpawnSaplings(Menu parent, Plugin plugin, Player player, boolean allowNull, Integer defaultValue
            , Consumer<Integer> setFunc) {
        ...
        add(new Button(3, makeItem(Material.RED_CONCRETE).setDisplayName("No, only replant if the leaves drop " +
                "saplings"), (click) -> {
            if (click != ClickType.LEFT) return;
            value = 0;
            setFunc.accept(value);
        ...
        add(new Button(4, makeItem(Material.YELLOW_CONCRETE).setDisplayName("Yes, but only if the leaves do not drop " +
                "enough"), (click) -> {
            if (click != ClickType.LEFT) return;
            value = 1;
            setFunc.accept(value);
        ...
        add(new Button(5, makeItem(Material.GREEN_CONCRETE).setDisplayName("Yes, always spawn new saplings"),
                (click) -> {
            if (click != ClickType.LEFT) return;
            value = 2;
            setFunc.accept(value);
