...
public class MenuModifyDouble extends Menu {
    ...
    public MenuModifyDouble(Menu parent, Plugin plugin, Player player, String name, double min, double max,
                            boolean allowNull, Double defaultValue, Consumer<Double> setFunc) {
        this(parent, plugin, player, name, min, max, 0.1, allowNull, defaultValue, setFunc);
    }

    public MenuModifyDouble(Menu parent, Plugin plugin, Player player, String name, double min, double max,
                            double majorIncrement, boolean allowNull, Double defaultValue, Consumer<Double> setFunc) {
        this(parent, plugin, player, name, min, max, majorIncrement, majorIncrement / 10, allowNull, defaultValue,
                setFunc);
    }

    public MenuModifyDouble(Menu parent, Plugin plugin, Player player, String name, double min, double max,
                            double majorIncrement, double minorIncrement, boolean allowNull, Double defaultValue,
                            Consumer<Double> setFunc) {
        super(parent, plugin, player, "Modify Double (" + name + ")", 9);
        value = defaultValue;
        display = add(new Button(0, makeItem(Material.PAPER).setDisplayName(Objects.toString(defaultValue)).addLore(
                "Click to set value"), (click) -> {
        ...
        if (allowNull) {
            ...
            add(new Button(4, makeItem(Material.BLACK_CONCRETE).setDisplayName("Set to " + Math.min(max, Math.max(min
                    , 0))), (click) -> {
                if (click != ClickType.LEFT) return;
    ...
    public void refresh() {
        display.label =
                makeItem(Material.PAPER).setDisplayName(Objects.toString(value)).addLore("Click to set value").build();
        updateInventory();
    }
}