...
public class MenuModifyTreeSet extends Menu {
    ...
    public MenuModifyTreeSet(Menu parent, Plugin plugin, Player player, String name, boolean allowNull,
                             Collection<Tree> defaultValue, Consumer<HashSet<Tree>> setFunc) {
    ...
    private static int getSize(boolean allowNull) {
        ...
        if (actualCount > 54)
            throw new IllegalArgumentException("MenuModifyTrees only supports up to " + (allowNull ? 51 : 52) + " " +
                    "values!");//TODO fix
    ...
    private void refresh() {
        ...
        for (int i = 0; i < Math.min(allowNull ? 51 : 52, TreeFeller.trees.size()); i++) {
            ...
            add(new Button(i + 1, makeItem(tree.getDisplayMaterial()).setDisplayName("Tree #" + i).addLore("Left " +
                    "click to add this tree").addLore("Right click to remove this tree").addLore(added ? "Added" :
                    "Not Added"), (click) -> {
        ...
        if (allowNull) {
            add(new Button(TreeFeller.trees.size() + 1, makeItem(Material.BLACK_CONCRETE).setDisplayName("Set to " +
                    "NULL"), (click) -> {
