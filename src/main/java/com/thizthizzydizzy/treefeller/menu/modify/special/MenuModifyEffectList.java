...
public class MenuModifyEffectList extends Menu {
    ...
    public MenuModifyEffectList(Menu parent, Plugin plugin, Player player, String name, boolean allowNull,
                                ArrayList<Effect> defaultValue, Consumer<ArrayList<Effect>> setFunc) {
    ...
    private static int getSize(boolean allowNull) {
        ...
        if (actualCount > 54)
            throw new IllegalArgumentException("MenuModifyEffects only supports up to " + (allowNull ? 51 : 52) + " " +
                    "values!");//TODO fix
    ...
    private void refresh() {
        ...
        for (int i = 0; i < Math.min(allowNull ? 51 : 52, TreeFeller.effects.size()); i++) {
            ...
            add(new Button(i + 1,
                    makeItem(effect.type.getItem()).setDisplayName(effect.name).setCount(count).addLore("Left click " +
                            "to add this effect").addLore("Right click to remove this effect").addLore("Currently " +
                            "added: " + count), (click) -> {
        ...
        if (allowNull) {
            add(new Button(TreeFeller.effects.size() + 1, makeItem(Material.BLACK_CONCRETE).setDisplayName("Set to " +
                    "NULL"), (click) -> {
