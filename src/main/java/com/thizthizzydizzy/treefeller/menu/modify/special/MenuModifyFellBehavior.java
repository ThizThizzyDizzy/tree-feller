...
public class MenuModifyFellBehavior extends Menu{//TODO make multi-page
    public MenuModifyFellBehavior(Menu parent, Plugin plugin, Player player, String name, boolean allowNull, FellBehavior defaultValue, FellBehavior[] values, Consumer<FellBehavior> setFunc){
        ...
        for(int i = 0; i<values.length; i++){
            int idx = i;
            add(new Button(i+1, makeItem(values[i].getItem()).setDisplayName("Set to "+Objects.toString(values[i])).addLore(shorten(values[i].getDescription(), 42)), (click) -> {
    ...
    private static int getSize(int count, boolean allowNull){
        ...
        if(actualCount>54)throw new IllegalArgumentException("MenuModifyFellBehavior only supports up to 52 values! (including null)");
