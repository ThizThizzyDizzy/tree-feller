...
public class MenuModifyFloat extends Menu {
    ...
    public MenuModifyFloat(Menu parent, Plugin plugin, Player player, String name, float min, float max,
                           boolean allowNull, Float defaultValue, Consumer<Float> setFunc) {
        this(parent, plugin, player, name, min, max, 0.1f, allowNull, defaultValue, setFunc);
    }

    public MenuModifyFloat(Menu parent, Plugin plugin, Player player, String name, float min, float max,
                           float majorIncrement, boolean allowNull, Float defaultValue, Consumer<Float> setFunc) {
        this(parent, plugin, player, name, min, max, majorIncrement, majorIncrement / 10f, allowNull, defaultValue,
                setFunc);
    }

    public MenuModifyFloat(Menu parent, Plugin plugin, Player player, String name, float min, float max,
                           float majorIncrement, float minorIncrement, boolean allowNull, Float defaultValue,
                           Consumer<Float> setFunc) {
