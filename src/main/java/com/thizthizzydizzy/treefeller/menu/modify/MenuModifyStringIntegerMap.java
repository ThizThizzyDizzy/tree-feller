...
public class MenuModifyStringIntegerMap extends Menu {
    ...
    public MenuModifyStringIntegerMap(Menu parent, Plugin plugin, Player player, String name, int min, int max,
                                      boolean allowNull, boolean allowNullIntegers,
                                      HashMap<String, Integer> defaultValue,
                                      Consumer<HashMap<String, Integer>> setFunc) {
            ...
            for (int i = 0; i < Math.min(value.size(), allowNull ? 51 : 52); i++) {
                ...
                add(new Button(i + 1,
                    ...
                    if (click == ClickType.LEFT) {
                        open(new MenuModifyInteger(this, plugin, player, "Modify " + strs.get(idx), min, max,
                                allowNullIntegers, value.get(strs.get(idx)), (value) -> {
