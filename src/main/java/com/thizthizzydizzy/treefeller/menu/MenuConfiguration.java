...
public class MenuConfiguration extends Menu {
    ...
    private void refresh() {
        ...
        for (int i = 0; i < size; i++) {
            ...
            add(new Label(i,
                    makeItem(Material.RED_STAINED_GLASS_PANE).setDisplayName(ChatColor.DARK_RED + "WARNING").addLore(
                            "All changes made are applied immediately!").addLore("To revert changes, run /treefeller " +
                            "reload").addLore("To save changes, click the save button" + additional)));
        }
        ...
        add(new Button(13,
                makeItem(Material.IRON_AXE).setDisplayName("Tools").addFlag(ItemFlag.HIDE_ATTRIBUTES).setCount(Math.max(1, TreeFeller.tools.size())), (click) -> {
            if (click == ClickType.LEFT) open(new MenuToolsConfiguration(this, plugin, player));
        }));
        add(new Button(15, makeItem(Material.OAK_SAPLING).setDisplayName("Trees").setCount(TreeFeller.trees.size()),
                (click) -> {
            if (click == ClickType.LEFT) open(new MenuTreesConfiguration(this, plugin, player));
        }));
        add(new Button(21,
                makeItem(Material.POTION).setDisplayName("Effects").setCount(TreeFeller.effects.size()).addFlag(ItemFlag.HIDE_POTION_EFFECTS), (click) -> {
            if (click == ClickType.LEFT) open(new MenuEffectsConfiguration(this, plugin, player));
        }));
        add(new Button(23, makeItem(Material.PAPER).setDisplayName("Messages").setCount(Message.messages.size()),
                (click) -> {
            if (click == ClickType.LEFT) open(new MenuMessagesConfiguration(this, plugin, player));
        }));
        add(new Button(size - 14, makeItem(Material.GREEN_CONCRETE).setDisplayName("Save Configuration").addLore(
                "Shift-left click to save.").addLore("This will overwrite the config.yml file!"), (click) -> {
