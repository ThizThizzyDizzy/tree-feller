...
public class MenuModifyEnum<T> extends Menu{
    public MenuModifyEnum(Menu parent, Plugin plugin, Player player, String name, String enumName, boolean allowNull, T defaultValue, T[] values, Consumer<T> setFunc){
        ...
        for(int i = 0; i<values.length; i++){
            int idx = i;
            add(new Button(i+1, new ItemBuilder(getItem(values[i])).setDisplayName("Set to "+Objects.toString(values[i])), (click) -> {
                if(click!=ClickType.LEFT)return;
                setFunc.accept(values[idx]);
                label.label = new ItemBuilder(Material.PAPER).setDisplayName(Objects.toString(values[idx])).build();
                updateInventory();
            }));
        }
        if(allowNull){
            add(new Button(values.length+1, new ItemBuilder(Material.BLACK_CONCRETE).setDisplayName("Set to NULL"), (click) -> {
                if(click!=ClickType.LEFT)return;
                setFunc.accept(null);
